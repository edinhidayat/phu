      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col">
            ...
            <div class="card">
              <div class="card-body">
                <a href="/dash/user/create" class="btn btn-primary mb-2"><i class="fas fa-plus"></i> Tambah Data</a>
                  ...
                  <tbody>
                    @foreach ($users as $item)
                      <tr>
                        ...
                        <td>{{ $item->nama }}</td>
                        <td>{{ $item->username }}</td>
                        <td>{{ $item->pengguna->pengguna }}</td>
                        <td>{{ $item->active }}</td>
                        <td class="text-center">
                          <a href="/dash/user/{{ $item->id }}/edit" style="text-decoration: none;"
                            class="btn btn-warning"><i class="fas fa-edit"></i> Ubah</a>
                          <form action="/dash/user/{{ $item->id }}" method="post" class="d-inline">
                            @method('delete')
                            @csrf
